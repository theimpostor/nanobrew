...
This repository contains local installer scripts and a consolidated installer (`nanobrew.sh`) that targets `~/.local`.

## Project layout
- `nanobrew.sh` is the primary, single-file installer CLI. Keep all package logic in this file.
- `design.md` and `prompt.txt` are design notes.
...
- Package functions live in `nanobrew.sh` using the naming convention:
  ...
  - Package names in the CLI use dashes; function names map `-` → `_`.
